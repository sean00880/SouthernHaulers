...
function NavigationMenuViewport({ className, ...props }: React.ComponentProps<typeof NavigationMenuPrimitive.Viewport>) {
    return (
        <div className={cn('px-(--viewport-outer-px) pointer-events-none fixed inset-x-0 top-20 isolate z-50 mx-auto flex max-w-6xl')}>
            <NavigationMenuPrimitive.Viewport
                data-slot="navigation-menu-viewport"
                className={cn('bg-popover text-popover-foreground h-(--radix-navigation-menu-viewport-height) ring-foreground/10 rounded-(--radius) md:w-(--radix-navigation-menu-viewport-width) pointer-events-auto relative mt-2 w-full origin-top overflow-hidden border border-transparent p-0.5 shadow-xl shadow-black/10 ring-1 transition-[width,height] duration-200', 'data-[state=closed]:animate-scale-out data-[state=open]:animate-scale-in', className)}
                {...props}
            />
        </div>
    )
